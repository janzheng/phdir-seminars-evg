...
{#if ticketPrice > 0}
  <div id="signup-finalize" class="{classes}">
    ...
    <div class="pricing _card _padding _color-bg-white">
      ...
      {#if paymentKey}
        {#if !hasStripe}
          <div class="_margin-bottom">Loading Stripe checkout</div>
        {:else if confirmingPayment}
          <div class="_margin-bottom">Confirming Payment</div>
        {:else}
        <!-- nothing -->
        {/if}
      {/if}
      ...
      <div class="_margin-bottom">
        <div>
          ...
          <div id="card-element" class="Stripe field input input_card _border-gr-icicle _margin-none-i" style="padding-top: 12px;"></div>
          <div id="card-errors" class="Stripe-error" role="alert"></div>
          <div class="_button _width-full __action _margin-top-half" on:click="{handleStripe}">Pay & Complete Registration</div>
          <div class="_font-small _padding-top-half">Payments powered by <a class="_font-small" href="https://stripe.com/">Stripe</a></div>
        </div>
      </div>
...
<script>
  ...
  async function preInit() {
    ...
    if(!paymentKey) {
      ...
      const res = await fetch(
        `/api/payments/getters?mode=stripe`, {
        method: 'GET',
      })
      if(res.ok) {
  ...
  let hasPaypal, hasStripe, signupData, confirmingPayment=false, confirmedPayment=false
  ...
  const handleStripe = async () => {
    ...
    // register completed payment w/ Airtable 
    const completeStripe = await fetchPost('/api/setters', { 
      data: {
        ...user,
      },
      type: 'complete_stripe'
    }, fetch)

    if(!completeStripe.ok) {
      console.error('Stripe Payment failed:', completeStripe.status, completeStripe)
      trail(`[SignupFinalizeStripe] Error: Payment/user update failed: ${user.name} - ${user.email}`, user)
            
      errorMsg = `Evergreen Stripe payment failed: ${payConfirmRes.status}`
      throw new Error('Evergreen Stripe payment failed')
      return
    }

    let stripejson = await completeStripe.json()
    signupData = stripejson['data']
    ...
    if (stripePayment.error) {
      ...
      trail(`[SignupFinalizeStripe] Stripe error: ${user.name} - ${user.email}`, stripePayment.error.message)
      ...
      const payConfirmRes = await fetchPost('/api/setters', { 
        ...
        regstatus: ['Attendee'],
        type: 'post_payment_stripe',
      }, fetch)

      if(!payConfirmRes.ok) {
        console.error('Payment confirmation error:', payConfirmRes.status, payConfirmRes)
        trail(`[SignupFinalizeStripe] Stripe error: ${user.name} - ${user.email}`, payConfirmRes.status)
      ...
      trail(`[SignupFinalizeStripe] Success: Payment + user update success: ${user.name} - ${user.email}`, user)
      logstream.log('SignupFinalizeStripe', `Payment + user update success: ${user.name} - ${user.email}`, user)
      zzz(scrollToAnchor, 'top', 200)
